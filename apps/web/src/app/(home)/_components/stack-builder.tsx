...
const analyzeStackCompatibility = (stack: StackState): CompatibilityResult => {
	...
	if (isConvex) {
		...
		if (nextStack.database === "none") {
			...
			if (nextStack.payments === "true") {
				notes.database.notes.push(
					"Database 'None' selected: Payments will be disabled.",
				);
				notes.auth.notes.push(
					"Payments requires a database. It will be disabled.",
				);
				notes.database.hasIssue = true;
				notes.payments.hasIssue = true;
				nextStack.payments = "false";
				changed = true;
				changes.push({
					category: "payments",
					message: "Payments disabled (requires a database)",
				});
			}
		...
		for (const category of CATEGORY_ORDER) {
			...
			for (const tech of options) {
				...
				if (rules.isBackendNone) {
					...
					} else if (catKey === "payments" && techId === "true") {
						addRule(category, techId, "Disabled: Payments requires a backend.");
					}
				}
				...
				if (catKey === "payments" && techId === "true") {
					if (stack.database === "none" && !rules.isBackendNone) {
						addRule(
							category,
							techId,
							"Disabled: Payments requires a database.",
						);
					}
				}
