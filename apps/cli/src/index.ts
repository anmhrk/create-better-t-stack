	...
	try {
		const argv = await yargs(hideBin(process.argv))
			...
			.option("payments", {
				type: "boolean",
				describe:
					"Include Stripe payments integration (use --payments true to include)",
			})
			.option("frontend", {
				type: "array",
				string: true,
		...
		if (options.yes) {
			...
			if (config.backend === "convex") {
				...
				config.examples = [];
				log.info(
					"Due to '--backend none', the following options have been automatically set: --auth=false, --payments=false, --database=none, --orm=none, --api=none, --runtime=none, --db-setup=none, --examples=none",
				);
				...
				config.payments = false;
				log.info(
					"Due to '--database none', '--payments' has been automatically set to 'false'.",
				);
...
function processAndValidateFlags(
	...
	if (config.backend === "convex") {
		...
		if (providedFlags.has("auth") && options.auth === true)
			incompatibleFlags.push("--auth");
		if (providedFlags.has("payments") && options.payments === true)
			incompatibleFlags.push("--payments");
		if (providedFlags.has("database") && options.database !== "none")
			incompatibleFlags.push(`--database ${options.database}`);
		if (providedFlags.has("orm") && options.orm !== "none")
			incompatibleFlags.push(`--orm ${options.orm}`);
		...
		const effectivePayments =
			config.payments ?? (options.yes ? DEFAULT_CONFIG.payments : undefined);
		...
		if (effectiveDatabase === "none") {
			...
			if (providedFlags.has("payments") && options.payments === true) {
				consola.fatal(
					"Payments require a database. Cannot use --payments when database is 'none'.",
				);
				process.exit(1);
			}
			config.payments = false;
